...
public interface HealthArticleMapper  {
    @Insert("INSERT INTO o_health_articles(article_id,doctor_id,title,content,publish_date,type,status)"
...
@Results(
    ...
    @Update("UPDATE o_health_articles SET doctor_id=#{doctorId},title=#{title},content=#{content},publish_date=#{publishDate},type=#{type},status=#{status} WHERE article_id=#{articleId}")
