...
public class UserLoginController {
  ...
  @Autowired private WeChatDecryptUtil weChatDecryptUtil; // 注入解密工具类
  ...
  public ResponseEntity<String> login(@RequestBody Map<String, String> body) {
    ...
    if (!weChatResponse.containsKey("openid") || !weChatResponse.containsKey("session_key")) {
      return ResponseEntity.badRequest()
          .body("Failed to get openid or session_key from WeChat API");
    }
    ...
    try {
      ...
      return ResponseEntity.ok(
          "{\"openid\":\"" + user.getOpenid() + "\",\"token\":\"" + jwtToken + "\"}");
    } catch (Exception e) {
      return ResponseEntity.badRequest().body("Failed to create user or update user information");
    }
  }

  private Map<String, String> getOpenIdAndSessionKeyFromWeChat(
    ...
    try {
      ...
      String session_key =
          jsonNode.get("session_key") != null ? jsonNode.get("session_key").asText() : null;

      if (openid == null || session_key == null) {
        throw new RuntimeException(
            "Failed to get openid or session_key from WeChat API response: " + response);
      }
      // 控制台输出
